...
function runPython(workspacePath: string, args: string[]): Promise<RunResult> {
  return new Promise((resolve, reject) => {
    const py = spawn("python", ["-m", "atlas", ...args], {
      cwd: workspacePath,
      env: { ...process.env }
    });
...
function readAtlasFile(workspacePath: string, rel: string, fallback = ""): string {
...
function getAtlasCounts(workspacePath: string): AtlasCounts {
  ...
  const archNodes =
    archData && typeof archData === "object" && Array.isArray((archData as { nodes?: unknown }).nodes)
      ? (archData as { nodes: unknown[] }).nodes.length
      : 0;
  const archEdges =
    archData && typeof archData === "object" && Array.isArray((archData as { edges?: unknown }).edges)
      ? (archData as { edges: unknown[] }).edges.length
      : 0;
...
function renderWebview(panel: vscode.WebviewPanel, workspacePath: string, lastPrompt: string) {
...
export function activate(context: vscode.ExtensionContext) {
  const getWorkspacePath = () => {
    ...
    if (!ws) throw new Error("Open a folder/workspace first.");
    return ws;
  };
  ...
  const ensurePanel = (workspacePath: string) => {
    if (panel) {
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
      if (!message || message.type !== "runPrompt") return;
  ...
  const runPromptFlow = async (workspacePath: string, prompt: string) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("atlas.initProject", async () => {
      try {
        const ws = getWorkspacePath();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("atlas.runPrompt", async () => {
      const ws = getWorkspacePath();
      ensurePanel(ws);
